# LEF Climbing — Wix → Cloudflare Migration Guide

This guide walks you through migrating the live `lefclimbing.com` site from Wix (hosting + DNS + registrar) to Cloudflare (hosting + DNS) with Dreamhost as the new registrar. Email on Google Workspace is preserved throughout.
...
1. Wix dashboard → **Domains** → `lefclimbing.com` → **Advanced** → **Edit DNS**
2. For every record (especially `A`, `CNAME`, `MX`, `TXT`), change TTL → **5 minutes** (300 seconds)
3. Save

**Verification:**
```bash
dig A lefclimbing.com | awk '/^lefclimbing\.com\./ {print $2 " seconds"}'
# Expect: 300 (or close — propagation takes a bit)
```

### 1.2 Final QA on the new site (15 min)

The new site is at `https://lef-climbing.chris-shotwell.workers.dev/` (until DNS cuts over).
...
1. https://dash.cloudflare.com → **Add a Site** → enter `lefclimbing.com` → **Continue**
...
3. Cloudflare scans Wix DNS and imports records. **Critically: verify all 5 Google Workspace MX records are imported:**
   ```
   lefclimbing.com  MX  1   aspmx.l.google.com
   lefclimbing.com  MX  5   alt1.aspmx.l.google.com
   lefclimbing.com  MX  5   alt2.aspmx.l.google.com
   lefclimbing.com  MX  10  alt3.aspmx.l.google.com
   lefclimbing.com  MX  10  alt4.aspmx.l.google.com
   ```
...
- Add TXT record:
  - Name: `_dmarc`
  - Value: `v=DMARC1; p=none; rua=mailto:info@lefclimbing.com`
...
1. Wix dashboard → **Domains** → `lefclimbing.com` → **Domain Settings**
...
1. Wix dashboard → **Domains** → `lefclimbing.com` → **Advanced** → **Nameservers**
...
1. Cloudflare dashboard → **Workers & Pages** → your `lef-climbing` project → **Settings** → **Domains & Routes** (or **Custom domains**) → **Add**
2. Enter `lefclimbing.com` → **Add domain**
3. Cloudflare creates the DNS record automatically and starts SSL provisioning (1–5 min)
4. Repeat: **Add** `www.lefclimbing.com` → Cloudflare creates the matching record
...
- [ ] `https://lefclimbing.com/`
- [ ] Every nav link
- [ ] One form submission

**Email test (critical):**
- [ ] Send an email from a non-LEF address to `info@lefclimbing.com`
- [ ] Wait ~1 minute
- [ ] Verify it arrives in the owner's Gmail inbox
...
2. Name: `lef-climbing-harden`
3. Permissions:
   - `Zone` → `Zone Settings` → **Edit**
   - `Zone` → `Zone` → **Read**
   - `Account` → `Account Rulesets` → **Edit**
4. Zone Resources: **Include** → **Specific zone** → `lefclimbing.com`
...
2. Enter `https://lefclimbing.com/` → **Continue**
...
6. Once verified:
   ...
   - **URL Inspection** (left nav) → enter `https://lefclimbing.com/` → **Request indexing**
   - Repeat for the most important pages (about, booking, membership, contact)

Google typically reindexes within 24–72 h. Old `/details`, `/instruction`, `/events` URLs in search results will gradually update to `/about`, `/classes`, `/calendar` via the 301 redirects.

Optional: also add `https://www.lefclimbing.com/` as a separate property to track its 301-redirect behavior.
...
You can check progress at Dreamhost → Domains → look for `lefclimbing.com` with status `In Progress`, then `Active`.
...
- [ ] `lefclimbing.com/instruction` → 301 to `/classes`
- [ ] `lefclimbing.com/events` → 301 to `/calendar`
- [ ] `lefclimbing.com/the-experience` → 301 to `/about`
- [ ] `lefclimbing.com/youth-classes` → 301 to `/classes`

**Email:**
- [ ] External email to `info@lefclimbing.com` arrives in her Gmail
- [ ] Send a test email FROM the LEF mailbox to verify outbound still works
- [ ] Check spam folder — should NOT be flagged

**Performance:**
- [ ] [PageSpeed Insights](https://pagespeed.web.dev) on `https://lefclimbing.com/` — should show ~99 mobile (matching what we saw on workers.dev)

**SSL/security:**
- [ ] Browser address bar shows the lock icon
- [ ] [SSL Labs Test](https://www.ssllabs.com/ssltest/) on `lefclimbing.com` — should grade A or A+
...
|---|---|---|
| `CLOUDFLARE_API_TOKEN` | Token with Zone:Read + Zone Settings:Edit + Account Rulesets:Edit on `lefclimbing.com` | https://dash.cloudflare.com/profile/api-tokens |

### Critical preserved DNS records (Google Workspace email)

These MUST be present in Cloudflare DNS after Phase 1.3. Without them, all email to `@lefclimbing.com` stops working:
...
1. Sign in to Dreamhost panel → **Domains → Manage Domains** → `lefclimbing.com` → **DNS** or **Nameservers**.
...
3. Run `dig NS lefclimbing.com +short` until it returns Cloudflare names (5–60 min).
