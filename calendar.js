// LEF Climbing — events calendar (loaded only on calendar.html).
...
(function () {
  ...
  function render() {
    ...
    if (metaEl) {
      if (STATE.updatedAt) {
        ...
        metaEl.textContent = `Calendar updated ${when}. Registration runs through portal.lefclimbing.com.`;
      } else {
        metaEl.textContent = 'Registration runs through portal.lefclimbing.com.';
      }
    }
  }
  ...
  function openEventModal(ev, fromList) {
    ...
    if (modalCta) {
      modalCta.href = ev.url || 'https://portal.lefclimbing.com/lef';
  ...
  fetch('/api/events', { cache: 'default' })
    ...
    .catch((err) => {
      ...
      if (statusEl) {
        statusEl.textContent = "We can't load the events calendar right now. Try the full LEF portal below, or reload the page.";
        statusEl.hidden = false;
      }
      render();
    });
