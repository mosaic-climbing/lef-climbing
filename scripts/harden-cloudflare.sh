#!/usr/bin/env bash
# harden-cloudflare.sh — one-shot Cloudflare zone hardening for lefclimbing.com.
#
...
#        Zone Resources: Include → Specific zone → lefclimbing.com
...
ZONE_NAME="${ZONE_NAME:-lefclimbing.com}"
...
echo
echo "→ Adding apex → www 301 redirect rule"
...
REDIRECT_DESC="apex to www (301) — auto-managed by harden-cloudflare.sh"
...
# Build the new rule list: existing rules minus any prior apex→www, plus our rule.
...
filtered.append({
    ...
    "action_parameters": {
        "from_value": {
            "status_code": 301,
            "target_url": {
                "expression": f'concat("https://www.{zone}", http.request.uri.path)'
            },
...
if [[ "$rr_ok" == "True" ]]; then
  echo "  ✓ $ZONE_NAME → www.$ZONE_NAME (301)"
else
